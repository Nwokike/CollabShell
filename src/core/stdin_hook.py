...
def show_stdin_dialog(page: ft.Page, prompt, snack_func=None, **kwargs):
    """Interactive dialog for kernel/GCP OAuth stdin requests."""
    ...
    logger.info("[stdin_hook] Prompt requested: %s", prompt_str)
    ...
    def _close_dialog(success=True, message=None):
        ...
        dialog.open = False
        page.update()
        if snack_func and success and is_ephemeral_drive_oauth:
            snack_func("✅ Google Drive authorized successfully!")
        elif snack_func and not success and message:
            snack_func(f"❌ {message}")
...
def setup_global_stdin_hook(page: ft.Page, colab_service, snack_func):
    colab_service.default_stdin_hook = lambda prompt, *args, **kwargs: (
        show_stdin_dialog(page, prompt, snack_func, *args, **kwargs)
    )
