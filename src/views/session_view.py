...
from core.styles import glass_card, section_header, hardware_badge, status_dot, build_banner_ad, tip_text
...
def build_session_view(
    ...
    if not session:
        return ft.Column(
            controls=[
                ...
                ft.Container(
                    content=ft.Text(constants.ERR_NO_SESSION, color=ft.Colors.ON_SURFACE_VARIANT),
    ...
    status_header = glass_card(
        ft.Column(
            controls=[
                ...
                ft.Text(
                    ...
                    size=tokens.FONT_SM,
                    color=AppColors.SUCCESS if is_running else ft.Colors.ON_SURFACE_VARIANT,
                ),
                ft.Text(
        ...
        margin=ft.Margin(tokens.SPACE_LG, tokens.SPACE_SM, tokens.SPACE_LG, tokens.SPACE_SM),
    )
    ...
    def _action_card(icon, label, on_click, color=None):
        return ft.Container(
            content=ft.Column(
                controls=[
                    ft.Icon(icon, size=tokens.ICON_LG, color=color or ft.Colors.PRIMARY),
    ...
    async def _on_files(e):
        page.go(f"/files?session={session_name}")

    async def _on_open_browser(e):
        try:
            url = await colab_service.get_session_url(session_name, auth_method=state.auth_method)
    ...
    async def _on_restart(e):
        ...
        confirm_dialog = ft.AlertDialog(
            ...
            content=ft.Text("This will restart the Python kernel. All variables will be lost."),
            actions=[
    ...
    async def _do_restart():
        page.open(ft.SnackBar(content=ft.Text("Restarting kernel...")))
        page.update()
        try:
            await colab_service.restart_kernel(session_name, auth_method=state.auth_method)
            page.open(ft.SnackBar(content=ft.Text("✅ Kernel restarted")))
        except Exception as ex:
    ...
    async def _do_stop():
        page.open(ft.SnackBar(content=ft.Text("Stopping session...")))
        page.update()
        try:
            await colab_service.stop_session(session_name, auth_method=state.auth_method)
            page.open(ft.SnackBar(content=ft.Text("✅ Session terminated")))
    ...
    async def _on_install(e):
        ...
        async def _do_install(ev):
            pkgs = pkg_field.current.value.strip()
            if not pkgs:
                return
            ...
            try:
                await colab_service.install_packages(
                    packages, session_name, auth_method=state.auth_method,
                    on_output=lambda t: _append_output(t),
                )
                page.open(ft.SnackBar(content=ft.Text("✅ Packages installed")))
            except Exception as ex:
                page.open(ft.SnackBar(content=ft.Text(f"❌ {ex}")))
        ...
        install_dialog = ft.AlertDialog(
            ...
            content=ft.Column(
                controls=[
                    ...
                    tip_text("Separate multiple packages with spaces. Uses uv if available, else pip."),
                ],
                tight=True,
                spacing=tokens.SPACE_SM,
            ),
            actions=[
                ft.TextButton("Cancel", on_click=lambda e: page.close(install_dialog)),
                ft.FilledButton("Install", on_click=lambda e: page.run_task(_do_install, e)),
            ],
        )
    ...
    async def _on_mount_drive(e):
        ...
        page.open(ft.SnackBar(content=ft.Text("Mounting Google Drive...")))
        page.update()
        try:
            await colab_service.mount_drive(
                session_name, path=state.drive_mount_path,
                auth_method=state.auth_method,
                on_output=lambda t: _append_output(t),
            )
            page.open(ft.SnackBar(content=ft.Text("✅ Drive mounted")))
        except Exception as ex:
            page.open(ft.SnackBar(content=ft.Text(f"❌ {ex}")))
    ...
    async def _on_auth_gcp(e):
        output_lines.clear()
        page.open(ft.SnackBar(content=ft.Text("Authenticating with GCP on VM...")))
        page.update()
        try:
            ...
            page.open(ft.SnackBar(content=ft.Text("✅ GCP auth complete")))
        except Exception as ex:
            page.open(ft.SnackBar(content=ft.Text(f"❌ {ex}")))
        page.update()

    async def _on_view_logs(e):
        page.go(f"/history?session={session_name}")
    ...
    action_grid = ft.Container(
        content=ft.Column(
            controls=[
                ft.Row(
                    controls=[
                        _action_card(ft.Icons.FOLDER_ROUNDED, "Files", lambda e: page.run_task(_on_files, e)),
                        _action_card(ft.Icons.DOWNLOAD_ROUNDED, "Install\nPackages", lambda e: page.run_task(_on_install, e)),
                        _action_card(ft.Icons.ADD_TO_DRIVE_ROUNDED, "Mount\nDrive", lambda e: page.run_task(_on_mount_drive, e)),
                    ],
                    spacing=tokens.SPACE_SM,
                ),
                ft.Row(
                    controls=[
                        _action_card(ft.Icons.SECURITY_ROUNDED, "Auth\nGCP", lambda e: page.run_task(_on_auth_gcp, e)),
                        _action_card(ft.Icons.OPEN_IN_BROWSER_ROUNDED, "Open in\nBrowser", lambda e: page.run_task(_on_open_browser, e), AppColors.BADGE_TPU),
                        _action_card(ft.Icons.HISTORY_ROUNDED, "View\nLogs", lambda e: page.run_task(_on_view_logs, e)),
                    ],
                    spacing=tokens.SPACE_SM,
                ),
                ft.Row(
                    controls=[
                        _action_card(ft.Icons.REFRESH_ROUNDED, "Restart\nKernel", lambda e: page.run_task(_on_restart, e), AppColors.WARNING),
                        _action_card(ft.Icons.STOP_CIRCLE_ROUNDED, "Stop\nSession", lambda e: page.run_task(_on_stop, e), AppColors.ERROR),
                    ],
                    spacing=tokens.SPACE_SM,
                ),
            ],
